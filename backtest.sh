...
declare full_profit=0.0
  for day in {01..30}
    ...
    do
        ...
        cat cp-config.js | sed -e "s/%coin/$coin/g; s/%year/2016/g; s/%month/08/g; s/%day_start/$day/g; s/%day_end/$day_end/g" > configs/$coin.config.js
    ...
    full_profit=$(awk '{print $1+$2}' <<<"$day_profit $full_profit")
done
echo $full_profit
